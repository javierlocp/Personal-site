function ExternalLink({ href, children, className = '', icon }) {
  ...
  return (
    <a href={href} target="_blank" rel="noopener" className={`${className} inline-block decoration-wavy decoration-0 underline-offset-2 hover:text-neutral-50 hover:underline`}>
      {children}
      {icon ?? defaultIcon}
    </a>
  );
